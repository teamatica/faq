...
<br/>
<br/>
### ❓ А где взять полностью готовый образ для Docker?

Готовый универсальный Docker-образ технически невозможен из-за требований к HTTPS: приложение использует валидное TLS-соединение с доменом организации, но сертификаты и доменные имена находятся в зоне ответственности администратора, а не разработчика.

Тем не менее, [индексный PHP-файл](https://github.com/teamatica/backend/blob/main/ru/%E2%9C%85%20PHP/index.php) можно использовать в любом окружении (включая Docker), если оно соответствует [системным требованиям](https://teamatica.org/#3).
<br/>
<br/>
<br/>
<br/>
...
<br/>
<br/>
### ❓ А можно скрыть конкретные аккаунты из общего списка, чтобы сделать их видимыми только для избранных?

Весь смысл приложения в том, чтобы обеспечивать всех пользователей всей информацией, поэтому здесь, к сожалению, скрыть никого не получится, но можно добавить учётные записи без указания контактной информации.
<br/>
<br/>
<br/>
<br/>
...
<br/>
<br/>
### ❓ А как после развёртывания завернуть всё обратно?

На сервере: удалить каталог "teamatica" в корне сайта.
<br/>
На клиенте: деинсталлировать приложение [как обычно](https://support.microsoft.com/ru-ru/windows/удаление-или-удаление-приложений-и-программ-в-windows-4b55f974-2cc6-2d2b-d092-5905080eaf98).
<br/>
<br/>
<br/>
<br/>
